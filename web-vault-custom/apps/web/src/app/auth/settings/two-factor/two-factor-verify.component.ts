...
export class TwoFactorVerifyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.mandatoryLockActive =
      this.lockService.isLockModeActive() && this.type === TwoFactorProviderType.Authenticator;
  ...
  submit = async () => {
    try {
      ...
      if (this.mandatoryLockActive) {
        this.lockService.forceCloseMandatoryDialog(this.dialogRef, {
          response: response,
